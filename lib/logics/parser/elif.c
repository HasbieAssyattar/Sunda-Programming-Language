...
void munteu(char* line){
    ...
    while (1){
        ...
        if(getType(token) != NULL && strcmp(getType(token), "string") == 0){ // jika string
            ...
            parserStrcmp(left, op, right, temp); // panggilnya yang operator strcmp
            strcat(output, temp);
        }else{ // kalau bukan string
            parserOperator(token, temp); // panggilnya operator biasa
            strcat(output, temp);
        }
